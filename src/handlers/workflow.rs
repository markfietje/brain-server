...
pub async fn get_run(
    ...
    let row: Option<RunRow> = tokio::task::spawn_blocking(move || {
        ...
        crate::workflow::state::run_row(&conn, id)
            .map(|opt| {
                opt.map(
                    |(id, domain, kind, status, state_json, created_at, updated_at)| RunRow {
                        id,
                        domain,
                        kind,
                        status,
...
pub async fn list_steps(
    ...
    let principal = principal.0;
    let pool = state.pool.clone();
    let (domain, steps): (Option<String>, Vec<serde_json::Value>) =
        tokio::task::spawn_blocking(move || -> Result<_, String> {
            let conn = pool.get().map_err(|e| format!("{e}"))?;
            let domain =
                crate::workflow::state::run_domain_of(&conn, id).map_err(|e| format!("{e}"))?;
            let Some(ref _d) = domain else {
                return Ok((None, vec![]));
            };
            let rows =
                crate::workflow::state::steps_of_run(&conn, id).map_err(|e| format!("{e}"))?;
            let out = rows
                .into_iter()
                .map(
                    |(id, run_id, phase, step_key, state_json, revision, parent_step_id)| {
                        serde_json::json!({
                            "id": id,
                            "run_id": run_id,
                            "phase": phase,
...
pub async fn post_steering(
    ...
    let domain: String = tokio::task::spawn_blocking(move || -> Result<String, String> {
        let conn = pool.get().map_err(|e| format!("{e}"))?;
        crate::workflow::state::run_domain_of(&conn, id)
            .map_err(|e| format!("{e}"))?
    ...
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        ...
        // The cap and the enqueue commit atomically: drop-oldest + enqueue in
        // one tx on one connection so the inbox bound can never race past 100.
        let tx = conn.transaction().map_err(|e| format!("{e}"))?;
        crate::workflow::outbox::enqueue_steering_tx(&tx, id, &domain, &payload, &actor)?;
...
pub async fn get_suggestions(
    ...
    let (domain, state_json): (String, String) =
        tokio::task::spawn_blocking(move || -> Result<_, String> {
            let conn = pool.get().map_err(|e| format!("{e}"))?;
            crate::workflow::state::run_domain_and_state(&conn, id)
                .map_err(|e| format!("{e}"))?
    ...
    let hits: Vec<serde_json::Value> = if q.is_empty() {
        ...
        tokio::task::spawn_blocking(move || -> Vec<serde_json::Value> {
            ...
            crate::workflow::kcs::reuse_candidates(&conn, &domain2, &q2, now)
                .into_iter()
...
pub async fn get_scoreboard(
    ...
    let runs: (
        ...
        let conn = pool.get().map_err(|e| format!("{e}"))?;
        let rows = crate::workflow::scoreboard::runs_page(&conn)?;
        let audited =
            crate::workflow::scoreboard::audited_run_ids(&conn, rows.iter().map(|(id, _, _)| *id));
        let aft = crate::workflow::scoreboard::aftersales_cohort(&conn)?;
        Ok((
            crate::workflow::scoreboard::derive_artifacts(&rows, &audited),
            aft,
        ))
    })
    .await
    ...
    let (risk, uniformity, ri_decisions) =
        tokio::task::spawn_blocking(move || -> Result<_, String> {
            let conn = pool_ri.get().map_err(|e| format!("{e}"))?;
            let now = chrono::Utc::now().timestamp();
            let (risk, ratio, approved, rejected) =
                crate::workflow::scoreboard::review_independence_measures(&conn, now)?;
    ...
    Ok(Json(serde_json::json!({
        ...
        "review_decisions_window": ri_decisions,
    })))
}
...
pub async fn post_calibration_sign(
    ...
    tokio::task::spawn_blocking(move || -> Result<serde_json::Value, HandlerError> {
        ...
        crate::workflow::calibration::record_signed(
            &tx,
            kappa,
            crate::workflow::scoreboard::score_units_now(&tx),
            &reviewer,
            now,
            &complaints,
        )
...
/// run row is written through [`crate::workflow::state::open_run`] under
/// [`crate::workflow::tx::WorkflowTx`] with the audit row in the SAME
/// transaction. Gated Write + the `workflow` role.
...
pub async fn get_run_state(
    ...
    let row: Option<(String, i64)> = tokio::task::spawn_blocking(move || -> Result<_, String> {
        let conn = pool.get().map_err(|e| format!("{e}"))?;
        let row = crate::workflow::state::read_state_and_revision(&conn, id)
            .map_err(|e| format!("{e}"))?;
        if row.is_some() {
...
pub async fn post_answer(
    ...
    let result: Result<i64, String> = tokio::task::spawn_blocking(move || -> Result<_, String> {
        ...
        let mut tx =
            crate::workflow::tx::WorkflowTx::begin(&mut conn).map_err(|e| format!("{e}"))?;
        let (js, rev): (String, i64) = crate::workflow::state::read_state_and_revision(tx.tx(), id)
            .map_err(|e| format!("{e}"))?
            // A run verified to exist moments ago (the domain probe) can only
            // vanish here through a concurrent delete — the same NoRows error
            // text the direct query produced, by the same route (500).
...
pub async fn get_steering(
    ...
    let rows: Vec<(i64, String)> = tokio::task::spawn_blocking(move || -> Result<_, String> {
        let conn = pool.get().map_err(|e| format!("{e}"))?;
        crate::workflow::outbox::steering_inbox(&conn, id, since).map_err(|e| format!("{e}"))
    })
    .await
    .map_err(|e| HandlerError::internal(format!("{e}")))?
    .map_err(HandlerError::internal)?;
