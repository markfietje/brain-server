...
This builds the release binaries, installs them to `~/.local/bin`, relocates the auth token to a 0600 file, restarts the service, and waits for `/health`. See **[Installation](Installation)** for details and the client GUI.

## Next steps

- Configure authentication and other tunables in **[Configuration](Configuration)**.
- Understand the retrieval pipeline in **[Architecture](Architecture)**.
- Review the security posture in **[Security](Security)**.
