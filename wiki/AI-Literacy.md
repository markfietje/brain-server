...
1. **Use the dashboard weekly.** Review the `/proposals` queue (approve / reject), check `/quarantine`, and read a couple of recall traces so the operator can state *why* a given answer was produced.
2. **Verify the chain on a schedule.** Run `/audit/verify` (or `brain status` / the `/metrics` chain-ok gauge) and keep the passing result as the audit evidence file.
3. **Run a DSAR drill before you need one.** Execute a purge against test subject data end to end (locate → export → purge → certificate) so the operator is literate in the deletion workflow before a real request arrives.

The dashboard, trace, approval queue, and DSAR console *are* the literacy surface — using them on a cadence is the evidence. For the machine-readable disclosure side, see the [Governance & Compliance](Governance-and-Compliance) page and `/.well-known/ai-notice`.

## Honest ceiling

This playbook documents **what the component makes inspectable and how to operate it**. Art 4 literacy for the *whole* AI system (the assistant an organization runs on top of Brain Server) is the deployer's broader program and is out of scope for a memory component — this page covers the component's slice and how to evidence it.
