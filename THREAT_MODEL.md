...
| Plugin-side transport smuggling (absolute-URL / protocol-relative path) | `BrainClient` pins `new URL(baseUrl).origin` at construction and refuses any request resolving outside it; stacks on the `assertSafeBaseUrl` scheme gate (https, or http only on loopback). Ceiling: DNS-rebind of the pinned host and never-seen-tool flagging (first-use not gated) remain accepted residuals — loopback-first deployments only | fork hardening (unreleased) |
