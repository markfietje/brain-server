...
| **I**nformation disclosure | Cross-tenant leak, PII egress | Per-tenant file isolation (v1.0); AuthZ layer (v1.2); record-level `access_scope` + owner (v1.14, JWT-mode deny-by-default filter); deterministic PII output redaction (v1.14, no write-time placeholder vault — v1.20.19); SQLCipher + per-field encryption (v3.7) | ✅ / 🚧 |
...
| 1.28.72 | 2026-09-08 | "Scrim" — `sanitize_read` gains `strip_hostile_elements` (closed case-insensitive element set) after the markdown-ref strip; storage verbatim so approval digests never move; `GET suggestions` evidence side-effect needs Write+workflow; denied `/events` subscribers get 403 before the stream opens; KB generator escapes operator-configured base_url/locales. |
| 1.28.71 | 2026-09-08 | "Pores" — the screen sees what the model sees: layer-1 runs on invisible-stripped text (bidi-split evasion closed, verdicts only tighten); translation families (es/de/fr/nl/fil) + typoglycemia + bounded encoding tiers replace the 13-phrase blocklist; optional ONNX classifier autoloads (fail-open, opt-out, `/health/db` echo); log values ANSI/C1-scrubbed; bridge edge strips + derefs before the clamp. |
| 1.28.70 | 2026-09-08 | "Twokeys" — the opaque-mode operator/agent split: token-file line 2 authenticates as scoped `AgentLoopback` (no Admin/purge/domains/revoke/dsar/DPO boards; writes proposal-gated under `review`); Blackout kill-switch revokes it by name; `/health/db` + `/metrics` scope per principal; single-token deployments keep the legacy posture with a boot warn. |
| 1.28.69 | 2026-09-08 | "Deadbolt" — the egress/process boundary: the shared egress client resolves→validates EVERY address against the IANA special-purpose table→pins insert-only (DNS-rebinding closed); alert/DSAR env sinks validate at boot, private sinks refuse without `BRAIN_EGRESS_ALLOW_PRIVATE=1`; harness binary is absolute-path only; spawned children die on drop; console `pending` requires the mapped actor's read capability. |
| 1.28.68 | 2026-09-07 | "Shutter" (docs + fork) — doc-mode remote images default OFF behind an exact-host allowlist; favicon proxy default OFF; `data:` URIs capped at 64 KiB decoded (the EchoLeak-class seats close in the openclaw fork; THREAT_MODEL §5 carries the surface table). |
| 1.28.67 | 2026-09-07 | "Pin" — parcels require `expected_signer` (400 `signer_required`); provenance verification pins the operator key (a foreign signer is visible, tampering reports Tampered under a pin); fork MCP catalog sha256-pinned per tool + per server, reconciled every run (drift notifies, drifted tools carry `pendingAck`); `BRAIN_MCP_SCOPE=read` denies the write verbs at dispatch; `/ump/audit/verify` gains the signed/hash-only integrity census. |
| 1.28.66 | 2026-09-07 | "Truthglass" — plugin approvals carry the EFFECTIVE tool-call arguments on both transports (display JSON, capped with exact-count markers); truncation keeps head AND tail unconditionally with count-first elision; `brain client dsar` requires `--action` and prompts with the subject digest; restore always prompts (`--force` skips only the liveness probe), passphrase files must be 0600. |
| 1.28.65 | 2026-09-07 | "Meridian" — content hygiene across the model seam: `/suggest` hits carry `untrusted: true`; the plugin's invisible set is EXACTLY the Rust canonical set (CI parity fixture); the openclaw merge seam sanitizes EVERY plugin context segment (invisible strip + forged-marker neutralization); MCP tool results ride the external-content envelope. |
| 1.28.64 | 2026-09-08 | "Blackout" — the principal kill-switch wires into bearer AuthN (revocation checked before signature/row work, probe-blind); denylist rows expire at the token's real `exp`; JWT per-kid alg compare; public-path lists deduplicated to ONE source; guard tables verify in BOTH directions; `INJECTION_POLICY=allow` warns loudly. |
| 1.28.63 | 2026-09-06 | "Wardline" — reserved topic vocabulary at the workflow input seam (`channel/*`, `steering`, `workflow/*` unreachable via the events route), closed run-status vocabulary, valet `what` screened on all write paths, alert-bus kind vocabulary closed — the three-gate channel law is code-true again. |
...
| 1.14.0 | 2026-08-07 | "Gate": human-in-the-loop write-back (`POST /ingest/proposal` → `POST /proposals/{id}/approve`, one tx, optional `?supersedes`); per-chunk `expires_at` decay (strict `<`, nothing decays autonomously); record-level `access_scope` + `owner` (JWT-mode deny-by-default filter); PII output redaction (`[redacted:…]`); GDPR `GET /export` + `POST /purge` (hard audited delete across tables, tombstone + audit) |
