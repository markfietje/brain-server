...
pub fn score_to_units(score: f64) -> Result<u32, String> {
    if !score.is_finite() || !(0.0..=1.0).contains(&score) {
        return Err("DI_INVALID_ARGUMENT".into());
    }
...
pub fn weighted_ambiguity_units(scores: &[f64], is_brownfield: bool) -> Result<u32, String> {
    let units: Vec<u32> = scores
        .iter()
        .map(|s| score_to_units(*s))
        .collect::<Result<_, _>>()?;
...
pub fn compute_ambiguity_floor(
    disputed: u32,
    unscored: u32,
    auto_scored: u32,
    scored: u32,
) -> FloorBreakdown {
    let auto_ratio = std::cmp::min(
        10000,
        (auto_scored * 10000).checked_div(scored).unwrap_or(0),
    );
    let floor = std::cmp::min(10000, disputed * 1000 + unscored * 500 + auto_ratio / 20);
    FloorBreakdown {
        disputed_count: disputed,
        unscored_active: unscored,
        auto_ratio_units: auto_ratio,
        floor_units: floor,
    }
}
...
pub fn derive_milestone(effective: u32, threshold: u32) -> Milestone {
    if effective <= threshold {
        Milestone::Ready
    } else if effective > 6000 {
        Milestone::Initial
    } else if effective > 3000 {
        Milestone::Progress
    } else {
        Milestone::Refined
    }
