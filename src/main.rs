...
use http_limit::{
    ConnectionTracker, RateLimiter, TrackerEntry, process_rss_mib, spawn_connection_watchdog,
    spawn_rss_watchdog,
};
...
mod server;
use server::router::auth::{JwtMiddlewareState, auth_middleware, jwt_auth_middleware};
use server::router::{rate_limit_middleware, request_id_middleware, security_headers_middleware};
...
async fn main_inner() -> Result<()> {
    ...
    boot::enforce_loopback_bind_guard(&addr, auth_mode)?;
