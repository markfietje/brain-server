...
| POST | `/webhooks/channel/{kind}/drain` | Bridge crank pulls approved outbound envelopes from the `channel/out` topic (approved acts or consented alert forwards ONLY; never the metadata-only alert bus, never SSE); batch marked delivered atomically. The drained `source_payload` now carries `kind` + `template` so edges deliver template acts as templates |
