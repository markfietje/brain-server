...
pub fn applicable_rule<'a>(
    ...
    rules
        .iter()
        .filter(|r| r.jurisdiction == jurisdiction && r.subject == subject && r.effective_at <= at)
...
pub fn apply_rate(
    ...
    if rule.reviewed_at.is_none() || rule.reviewed_at.unwrap() == 0 {
        return Err(ApplyError::MissingReview {
            rule_key: rule.rule_key.clone(),
        });
    }
    let rate = rates
...
pub fn seed_rules() -> (Vec<Rule>, Vec<(i64, Rate)>) {
    let rules = vec![
        Rule {
            ...
            body: "Standard VAT rate".into(),
            source_ref: "Wet OB 1968".into(),
            effective_at: 1704067200,
            reviewed_at: Some(1704067200),
            expires_at: None,
            revision: 1,
            superseded_by: None,
    ...
    let rates = vec![
        (1, Rate { basis_points: 2100 }),
        (2, Rate { basis_points: 1200 }),
    ];
    (rules, rates)
}
...
mod tests {
    ...
    fn rulebook_reconstructs_any_date() {
        ...
        rules.push(Rule {
            ...
            body: "Updated".into(),
            source_ref: "Wet OB 1968".into(),
            effective_at: 1706745600,
            reviewed_at: Some(1706745600),
            expires_at: None,
            revision: 2,
            superseded_by: None,
