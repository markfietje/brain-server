...
Every result carries `provenance`: per-retriever ranks, the fused score, and any expansion terms. With the **[Client GUI](Client-GUI)** you can open the recall decision-path viewer to see *why* each chunk was chosen — the per-retriever ranks, fused score, relevance tier, and source.
...
- **[Knowledge Graph](Knowledge-Graph)** — the graph layer that powers the optional leg.
- **[Architecture](Architecture)** — where retrieval sits in the whole system.
- **[API Reference](API-Reference)** — the exact request/response contract.
