//! Ingest capture hygiene: stop the server from silently storing model
//! reasoning traces and foreign synthesis prompts.
//!
//! Two pure, unit-tested transforms at the raw-text ingest doors
//! (`/ingest/memory`, `/add`):
//!   - [`strip_reasoning_blocks`]: removes paired reasoning-tag blocks
//!     (`<thinking>…</thinking>`, ` thinking`, `<reasoning>`, `<reflection>`,
...
//!
//! ponytail: an allow-list of tag names the audit proved leak, NOT a general
//! "AI-text" detector. Extend [`REASONING_TAGS`] as new delimiters appear; do
//! not build a content classifier. Curated ingest (`/ingest`, `/ingest/markdown`)
//! is deliberately NOT filtered here (operator-authored; false-positive risk).
//! This stops the bleeding; cleaning history is a separate ROADMAP sweep.

/// `<name>…</name>` block tag names stripped with their content.
/// Case-insensitive. Sourced from the round-5 audit: Anthropic `<thinking>`,
/// DeepSeek ` thinking`, plus common reasoning/reflection/analysis variants.
...
///
/// A block is `<tag …>` … `</tag>` (open tag may carry attributes); open tag,
/// content, and close tag are all removed. An open tag with **no** matching
/// close is treated as a truncated trace and dropped to end-of-string (the
/// conservative privacy choice — a cut-off transcript opening a reasoning
/// block is reasoning, not memory). Non-matching text passes through verbatim.
    ...
    while i < text.len() {
        if let Some((after_open, tag)) = match_open_tag(&lower, i) {
            ...
            if let Some(rel) = lower[after_open..].find(&close) {
                ...
                // Unclosed reasoning block: drop the rest.
                break;
            }
        } else {
...
/// Accepts `<tag>` and `<tag …>` (attributes), but rejects `<tagx>` so a longer
/// identifier isn't matched as a tag-name prefix.
...
/// True if `text` (ignoring leading whitespace) begins with any configured
/// `patterns`. Case-sensitive — dream/synthesis prompts are exact strings and
/// case-folding would widen false positives.
...
/// newlines or commas. Empty/unset → no patterns (opt-in; default unchanged).
/// Read per ingest — cheap, env is static over process life.
...
/// Combined door check for one ingest entry: returns `None` if dropped
/// (skip-pattern match), else `Some(cleaned_text)` with reasoning blocks
/// stripped. `patterns` is caller-supplied (handlers pass [`skip_patterns`])
/// so the logic is testable without touching the process env.
