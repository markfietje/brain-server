//! The monolith's test mass — moved VERBATIM out of `src/main.rs` at the
//! Capstone flip. main.rs is wiring-only now (≤ 300 lines,
//! pinned by `spire_inventory`), every route registration lives under
//! `src/server/router/**`, and this suite is the region the Spire Line
//! dismantled around. It addresses the lib through `brain_server::*` exactly
//! as it did when it lived at the binary's root; the only edits are the
//! `include_str!` anchors (now `CARGO_MANIFEST_DIR`-absolute, because this
//! file no longer sits beside its subjects) and the root use-block below,
//! which traveled with it from the old main.rs root so `use super::*`
//! resolves exactly as before.
//!
//! Tests are relocated, never deleted: the crate-wide `#[test]` floor in
//! `src/spire_inventory.rs` covers `src/` + `tests/` and never decreases.
...
/// credentials). Local twin of the lib's `testkeys::unit_hmac_key` —
/// integration tests cannot see the lib's `#[cfg(test)]` items.
#[cfg(test)]
...
Final paragraph after the rule.";
    ...
    fn ingest_insert_sites_write_owner_column() {
        ...
        let ingest_core_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/service/ingest.rs"
        ));
        ...
        let gate_src = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/handlers/gate.rs"));
        assert!(
    ...
    fn stored_text_fields_pass_the_read_seam() {
        ...
        let _main_src = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/main.rs"));
        let gate_src = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/handlers/gate.rs"));
        let suggest_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/handlers/suggest.rs"
        ));
        let recall_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/handlers/recall.rs"
        ));
        let ump_src = include_str!(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/src/handlers/ump_ops.rs"
        ));
    ...
    fn logout_wired_behind_bearer_and_denies_without_principal() {
        ...
        let auth_src = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/handlers/auth.rs"));
        assert!(
            auth_src
    ...
    async fn kb_feedback_webhook_requires_hmac_and_rejects_replay() {
        // Generated key (no literal secret anywhere): the file holds the
        // same generated bytes the headers are signed with.
        let relay_secret = unit_hmac_key(21);
        let secret_file = tempfile::NamedTempFile::new().unwrap();
        std::fs::write(secret_file.path(), &relay_secret).unwrap();
        let prev = {
            let _env = env_lock();
        ...
        // Bad signature → 401.
        let bad = kb_feedback_headers(&unit_hmac_key(22), "wh-1", &now, body);
        let resp = brain_server::handlers::webhooks::receive(
            axum::extract::State(state.clone()),
            axum::extract::Path("kb-feedback".into()),
            bad,
        ...
        let good = kb_feedback_headers(&relay_secret, "wh-2", &now, body);
        let resp = brain_server::handlers::webhooks::receive(
            axum::extract::State(state.clone()),
            axum::extract::Path("kb-feedback".into()),
            good.clone(),
    ...
    async fn console_seam_digest_law_and_actor_role_checks() {
        ...
        // Generated text-safe key (no literal secret anywhere): the JSON
        // file and the sign() helper below share the same generated bytes.
        let herald_secret: String = unit_hmac_key(23)
            .into_iter()
            .map(|b| {
                let v = b % 36;
                if v < 10 {
        ...
        std::fs::write(
            &cfg_path,
            format!("{{\"domain\":\"acme\",\"webhook_secret\":\"{herald_secret}\"}}"),
        )
        .unwrap();
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        fn sign(secret: &[u8], body: &[u8]) -> [String; 3] {
            use base64::Engine;
        ...
        fn call_with_secret(
            ...
            let [id, ts, sig] = sign(secret, &bytes);
        ...
        let resp = call_with_secret(
            Arc::clone(&state),
            herald_secret.as_bytes(),
            serde_json::json!({
