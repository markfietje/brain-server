...
| **COPC R8.0** performance assessment | the normative metric dictionary (`docs/metrics.md` + its schema-versioned twin `metrics/metrics.json`): every emitted metric carries formula · unit · source table.column lineage · window semantics · inclusion/exclusion rules · standard citation · tier availability; parity meta-tests pin docs↔code↔schema (`every_scoreboard_field_has_a_dictionary_entry`, `every_entry_source_table_exists_in_schema`, `formula_change_bumps_scorer_version`); FCR per the SQM repeat-window method (`BRAIN_FCR_WINDOW_DAYS`, default 7); benchmarks are reference points, never claims |
