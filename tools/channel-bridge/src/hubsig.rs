...
mod tests {
    ...
    fn hub_signature_verified_constant_time() {
        // Generated text-safe key (no literal secret anywhere) — computed AND
        // verified at runtime against the same generated bytes.
        let secret: String = crate::testkeys::unit_hmac_key(34)
            .into_iter()
            .map(|b| (b % 94 + 33) as char)
            .collect();
        ...
        assert!(!verify_hub_signature(
            crate::testkeys::unit_hmac_key(35).as_slice(),
            body,
            &good
        ));
        ...
        assert!(!verify_hub_signature(Vec::new().as_slice(), body, &good));
    }
