use super::state::{cas_update, CasError};
...
pub fn decide(state: &Value) -> Decision {
    let status = state.get("status").and_then(|v| v.as_str()).unwrap_or("active");
    if status == "done" || status == "complete" {
        return Decision::Done;
    }
    if let Some(q) = state.get("pending_question").and_then(|v| v.as_str()) {
        return Decision::AskHuman { question: q.to_string() };
    }
    if let Some(s) = state.get("next_step").and_then(|v| v.as_str()) {
        return Decision::RunStep { step: s.to_string() };
    }
    if let Some(n) = state.get("next_state").and_then(|v| v.as_str()) {
        return Decision::Advance { next_state: n.to_string() };
    }
    Decision::Done
}
...
pub fn advance(conn: &Connection, run_id: i64, expected_rev: i64, next_state: &str, now: i64) -> Result<(), CasError> {
    let v: Value = serde_json::from_str(next_state).unwrap_or(Value::String(next_state.to_string()));
    let s = v.to_string();
...
mod tests {
    ...
    fn decision_table_routes() {
        ...
        assert!(matches!(decide(&json!({"pending_question":"hi"})), Decision::AskHuman{..}));
        assert!(matches!(decide(&json!({"next_step":"inventory"})), Decision::RunStep{..}));
        assert!(matches!(decide(&json!({"next_state":"{}"})), Decision::Advance{..}));
    }
    ...
    fn orchestration_replays_after_crash_exactly_once() {
        ...
        assert!(matches!(advance(&conn,1,0,"{}",3), Err(CasError::Stale{..})));
